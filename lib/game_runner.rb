...
class GameRunner
  ...
  def show_grid(own = false, post_sleep = 0.5)
    sleep(1)
    clear
    if own
  ...
  def coord?(x)
    ...
    return false unless Array('A'..'Z').include?(x[0])
  ...
  def validate_placement(answer) # returns coords
    ...
    return false if !coord?(c1) || !coord?(c2)
    [c1, c2]
  end
  ...
  def prompt_fire(prev_msg = nil, sleep_time = 1, clear = false)
    ...
    answer = prompt_helper(msg, sleep_time, prev_msg, clear)
  ...
  def proceed_game_flow
    ...
    until endgame
      ...
      prompt_player2_shot_sequence unless endgame
    end
    proceed_endgame
  end
